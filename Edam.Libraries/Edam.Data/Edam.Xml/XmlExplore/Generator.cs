...
namespace Edam.Xml.XmlExplore {
    ...
    internal abstract class XmlValueGenerator {
        ...
        internal static XmlValueGenerator CreateGenerator(XmlSchemaDatatype datatype, int listLength) {
            ...
            {
               // TODO: debug CreateUnionGenerator
               generator = new Generator_string(rFacets); // CreateUnionGenerator(datatype, rFacets, listLength);
            }
        ...
        private static XmlValueGenerator CreateUnionGenerator(XmlSchemaDatatype dtype, CompiledFacets facets, int listLength) {
            XmlSchemaSimpleType[] memberTypes =
               (XmlSchemaSimpleType[])dtype.GetType().InvokeMember(
                  "types", BindingFlags.GetField | BindingFlags.NonPublic |
                  BindingFlags.Instance, null, dtype, null);
        ...
        internal class Generator_duration : XmlValueGenerator {
            ...
            TimeSpan minBound   = new TimeSpan(
               TimeSpan.MinValue.Days,   TimeSpan.MinValue.Hours, 
               TimeSpan.MinValue.Minutes,TimeSpan.MinValue.Seconds,
               TimeSpan.MinValue.Milliseconds);
            TimeSpan maxBound   = new TimeSpan(
               TimeSpan.MaxValue.Days,   TimeSpan.MaxValue.Hours,
      ...
      internal class Generator_dateTime : XmlValueGenerator {
            ...
            public override string GenerateValue() { 
                ...
                return String.Format("{0:u}", result);
            } 
